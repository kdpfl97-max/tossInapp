...
function getReactCompatScript() {
  const reactUsePolyfillPath = require.resolve("react18-use").split(require("path").sep).join("/");
  const reactEffectEventPolyfillPath = require.resolve("use-effect-event").split(require("path").sep).join("/");
  return `
