...
import { rally, useRally } from "@tds/react-native";
...
export default function ResultPage({
  ...
  const { ref, rally: rallyMotion } = useRally({
    delay: 0.5,
    motions: rally.scaleBounce({ type: "in", speed: "slow" }),
  });
  ...
  useEffect(() => {
    // RN 환경에선 showPointComponent(toast)로 노출되는 영역
    // 웹 로컬 테스트에선 간단한 토스트 UI로 대체
    ...
    <div className="rp-container">
      <div className="rp-navbar">
        ...
        <div className="rp-nav-actions" aria-label="상단 메뉴">
          <button className="rp-nav-btn" aria-label="좋아요">
            ♥
          </button>
          <button className="rp-nav-btn" aria-label="더보기">
            ···
          </button>
          <div className="rp-nav-divider" aria-hidden="true" />
          <button className="rp-nav-btn" aria-label="닫기" onClick={onClose}>
            ✕
          </button>
        </div>
      </div>
